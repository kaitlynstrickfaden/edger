...
#' @param contour_vals A vector of numeric values between 0-1 for the lowest contour value you want to recolor in each image(a low contour value captures weaker contrasts). Default is 0.1.
                       ...
                       cores = 1)
{
  ...
  if (any(contour_vals <= 0)) {
    stop("contour_vals must be a vector of positive non-zero numbers.")
  }
  ...
  roi <- ct_identify(ref_ims = ref_ims,
                     roi_in = roi_in,
  ...
  im1_df <- ct_im_to_df(im1)

  m <- ct_match(imdf = im1_df,
                roi = roi,
                shift = shift)

  im1_new <- ct_recolor(imdf = im1_df,
                        m = m,
                        rgbcolor = rgbcolor,
                        show = TRUE)

  im1_newname <- ct_name(images[1])

  ct_save(im1_new, im1_newname)
  ...
  if (process == "sequential") {
      ...
      # pb <- progress_bar$new(
      #   format = " recoloring image :current of :total  [:bar]  :percent  elapsed: :elapsedfull  ETA: :eta",
      #   total = length(images[-1]), clear = FALSE, width = 80)
